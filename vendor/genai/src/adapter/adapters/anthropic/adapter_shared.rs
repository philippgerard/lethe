...
/// Lethe fork: visibility widened from private so the OpenAI adapter can emit
/// the same wire format. OpenRouter relays Anthropic-style `cache_control` over
/// the OpenAI protocol, so both paths must agree on this mapping by
/// construction rather than by a copy that can drift.
pub(in crate::adapter::adapters) fn cache_control_to_json(cache_control: &CacheControl) -> Value {
	match cache_control {
