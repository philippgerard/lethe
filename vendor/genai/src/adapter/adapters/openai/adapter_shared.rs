...
// Lethe fork: reuse the Anthropic cache_control mapping — see the System arm of
// into_openai_request_parts() for why the OpenAI path emits it.
use crate::adapter::adapters::anthropic::cache_control_to_json;
...
impl OpenAIAdapter {
	...
	fn into_openai_request_parts(_model_iden: &ModelIden, chat_req: ChatRequest) -> Result<OpenAIRequestParts> {
		...
		for msg in chat_req.messages {
			...
			// Lethe fork: capture cache_control off the message before moving its
			// content. Upstream only reads cache_control at the request level, where
			// it becomes OpenAI's native `prompt_cache_retention`. That does not help
			// the route we care about: OpenRouter speaks the OpenAI protocol but
			// relays per-message `cache_control` on to the upstream vendor, which is
			// how prompt caching reaches Anthropic / Gemini / Qwen through it.
			// Direct OpenAI silently drops unknown fields, so this is safe on both
			// paths. See https://openrouter.ai/docs/features/prompt-caching
			// Cloned, not borrowed: CacheControl is a fieldless enum (and is no
			// longer Copy as of 0.6.0), and msg.content is moved just below.
			let msg_cache_control = msg.options.as_ref().and_then(|opts| opts.cache_control.clone());
			match msg.role {
						...
						if let Some(cache_control) = msg_cache_control {
							messages.push(json!({
								...
								"content": [{
									"type": "text",
									"text": content,
									"cache_control": cache_control_to_json(&cache_control),
								}],
							}));
						} else {
...
mod tests {
	...
	// region:    --- Lethe fork cache_control tests

	fn lethe_payload_for(messages: Vec<ChatMessage>) -> Value {
		let target = ServiceTarget {
			model: test_model(),
			auth: AuthData::from_single("test-key"),
			endpoint: Endpoint::from_static("https://api.openai.com/v1/"),
	...
	#[test]
	fn lethe_fork_system_message_without_a_marker_stays_a_plain_string() {
		let payload = lethe_payload_for(vec![ChatMessage::system("you are helpful")]);
		assert_eq!(payload["messages"][0]["role"], json!("system"));
		assert_eq!(payload["messages"][0]["content"], json!("you are helpful"));
	}

	#[test]
	fn lethe_fork_system_message_with_1h_marker_emits_cache_control_parts() {
		// This is the OpenRouter -> Anthropic prompt-cache route: OpenRouter
		// speaks the OpenAI protocol but relays this field to Anthropic.
		let mut msg = ChatMessage::system("stable prefix");
		msg.options = Some(CacheControl::Ephemeral1h.into());
		let payload = lethe_payload_for(vec![msg]);

		let content = payload["messages"][0]["content"]
			.as_array()
			.expect("content must become a parts array when a marker is present");
	...
	#[test]
	fn lethe_fork_system_message_with_default_marker_emits_5m_cache_control() {
		let mut msg = ChatMessage::system("volatile tail");
		msg.options = Some(CacheControl::Ephemeral.into());
		let payload = lethe_payload_for(vec![msg]);

		let content = payload["messages"][0]["content"].as_array().expect("parts array");
		assert_eq!(content[0]["cache_control"], json!({"type": "ephemeral"}));
	}

	#[test]
	fn lethe_fork_non_system_messages_keep_their_existing_shape() {
		let payload = lethe_payload_for(vec![ChatMessage::user("hello")]);
		assert_eq!(payload["messages"][0]["role"], json!("user"));
		assert_eq!(payload["messages"][0]["content"], json!("hello"));
	}

	// endregion: --- Lethe fork cache_control tests
}
