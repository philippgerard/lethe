...
- Prefer device/browser authorization flows that keep credentials out of the conversation. NEVER ask the principal to paste a token, password, API key, or other secret into chat or a terminal you can read. If a compatible `vault_add` secure-input flow is available, use it; otherwise tell the principal to configure the credential outside this conversation and report back only whether setup succeeded.
